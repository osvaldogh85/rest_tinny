...
public class DeleteCall extends Call {
    ...
    private DeleteCall(String serverURL, String requestMethodPath, String apiToken,Map<String,String> queryParameters,RequestType type) {
    ...
    public static ICall build(String serverURL, String requestMethodPath, String apiToken,Map<String,String> queryParameters ,RequestType type) {
        return new DeleteCall(serverURL, requestMethodPath, apiToken ,queryParameters,type);
    }
  ...
  public void prepareCall() {
    super.prepareCall();
    if(TYPE == AbstractCall.TYPE_JSON_QUERY || TYPE == AbstractCall.TYPE_HTTP_QUERY){
       super.addURLInfo(serverURL, requestMethodURL, Method.DELETE, TYPE);
       queryParameters.keySet().forEach(key -> {
           super.addQueryParameter(key, queryParameters.get(key));
        });
    }
    else{
       if(TYPE == AbstractCall.TYPE_JSON_BODY ){
